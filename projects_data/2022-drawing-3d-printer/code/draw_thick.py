...
def get_score(arr,test_v,rot_vec):
    ...
    for i in range(50):
        ...
        arr[int(p[0]):int(p[0])+5,int(p[1]):int(p[1])+5] = 255
...
def set_score(arr, test_v, rot_vec):
    sum_ = 0
    p = [101,101]
    for i in range(50):
        sum_ += 255-arr[int(p[0]),int(p[1])]
...
while True:
    ...
    if loop_nr%1000 == 1:
        cv2.imwrite("test" + str(loop_nr) + ".jpg", blank_image)
    for a in range(5):
        #blank_image[int(position[0]):int(position[0]+5),int(position[1]):int(position[1]+5)] = blank_image[int(position[0]),int(position[1])]*0.8
        blank_image[int(position[0]):int(position[0]+5),int(position[1]):int(position[1]+5)] = 0
        base[int(position[0]-5):int(position[0]+10),int(position[1]-5):int(position[1]+10)]  = 255 
        position += force
        #print(force)
